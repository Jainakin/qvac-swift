...
function selfTestNativeClosure() {
  ...
  // Ordinary freshness checks must keep grant/source review usable while an
  // explicit publication check is the fail-closed release boundary.
  ...
  if (!/- name: Require complete native-license and privacy closure before publication\n\s+run: \|\n\s+node tools\/release\/generate-third-party-notices\.mjs --check-publication\n/.test(sourceWorkflow)) {
...
function render() {
  ...
  const lines = [
    ...
    '> pinned supplement and native-component manifests under `tools/release/`.',
  ...
  if (nativeClosure.blockers.length === 0) {
    ...
    lines.push(
      ...
      'The grant source handoff may still be reviewed, but the binary artifacts must not',
      'be published until every item below is resolved and this generator passes',
      '`--check-publication`.',
      '',
    )
